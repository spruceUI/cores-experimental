...
from core_pipeline_lib.contracts import chimerasnes
...
class GwManifestTests(unittest.TestCase):
    ...
    def test_catalog_binds_exact_libretro_super_recipe(self) -> None:
        ...
        self.assertEqual("libretro-chimerasnes", self.spec["build"]["source_dir"])
        self.assertEqual(["arm64", "armhf"], self.spec["targets"])

    def test_spec_is_well_formed_pins_the_catalog_identity(self) -> None:
        self.assertTrue(chimerasnes.chimerasnes_spec_is_well_formed(self.spec))
        mutated = {**self.spec, "targets": ["arm64"]}
        self.assertFalse(chimerasnes.chimerasnes_spec_is_well_formed(mutated))
    ...
    def test_workflow_is_a_migrated_read_only_dispatcher(self) -> None:
        ...
        self.assertIn("--core chimerasnes", workflow)
...
class GwCompatibilityTests(unittest.TestCase):
    def test_promoted_compatibility_is_valid_and_canonical(self) -> None:
        compatibility_path = ROOT / "manifests/compatibility/chimerasnes.json"
        ...
        self.assertFalse(
            (ROOT / "manifests/compatibility/pending/chimerasnes.json").exists()
        )
...
class GwContractTests(unittest.TestCase):
        ...
        for run_id in (SELECTED_RUN, REPRODUCTION_RUN):
            for arch in ("arm64", "armhf"):
                ...
                if log is None:
                    continue
                self.assertTrue(
                    chimerasnes.chimerasnes_log_proves_contract(
    ...
    def test_contract_rejects_a_wrong_compile_count(self) -> None:
        ...
        tampered = dataclasses.replace(
            chimerasnes.CHIMERASNES_LOG_CONTRACT, expected_compile_count=999
        )
        self.assertFalse(
            c_only_log_proves_contract(
