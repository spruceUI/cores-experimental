"""Focused Fuse catalog, workflow, contract, and canonical-state tests."""
...
from core_pipeline_lib.contracts import fuse
...
class FuseManifestTests(unittest.TestCase):
    ...
    def test_catalog_binds_exact_libretro_super_recipe(self) -> None:
        ...
        self.assertEqual("libretro-fuse", self.spec["build"]["source_dir"])
        self.assertEqual(["arm64", "armhf"], self.spec["targets"])

    def test_spec_is_well_formed_pins_the_catalog_identity(self) -> None:
        self.assertTrue(fuse.fuse_spec_is_well_formed(self.spec))
        mutated = {**self.spec, "targets": ["arm64"]}
        self.assertFalse(fuse.fuse_spec_is_well_formed(mutated))
    ...
    def test_workflow_is_a_migrated_read_only_dispatcher(self) -> None:
        ...
        self.assertIn("--core fuse", workflow)
...
class FuseCompatibilityTests(unittest.TestCase):
    def test_promoted_compatibility_is_valid_and_canonical(self) -> None:
        compatibility_path = ROOT / "manifests/compatibility/fuse.json"
        ...
        self.assertFalse(
            (ROOT / "manifests/compatibility/pending/fuse.json").exists()
        )
...
class FuseContractTests(unittest.TestCase):
        ...
        for run_id in (SELECTED_RUN, REPRODUCTION_RUN):
            for arch in ("arm64", "armhf"):
                ...
                if log is None:
                    continue
                self.assertTrue(
                    fuse.fuse_log_proves_contract(
                        log, CORE_ID, arch, SOURCE_COMMIT, SOURCE_TREE
                    ),
                    f"{run_id}/{arch} did not prove the Fuse contract",
                )
                proven += 1
        if proven == 0:
            self.skipTest("no workspace-local Fuse build logs present")
    ...
    def test_contract_rejects_a_wrong_compile_count(self) -> None:
        ...
        if log is None:
            self.skipTest("no workspace-local Fuse build log present")
        tampered = dataclasses.replace(
            fuse.FUSE_LOG_CONTRACT, expected_compile_count=999
        )
        self.assertFalse(
            c_only_log_proves_contract(
