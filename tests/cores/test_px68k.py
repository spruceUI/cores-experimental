"""Focused px68k catalog, workflow, contract, and canonical-state tests."""
...
class Px68kManifestTests(unittest.TestCase):
    ...
    def test_catalog_binds_exact_libretro_super_recipe(self) -> None:
        ...
        self.assertEqual("libretro-px68k", self.spec["build"]["source_dir"])
        self.assertEqual(["arm64", "armhf"], self.spec["targets"])

    def test_spec_is_well_formed_pins_the_catalog_identity(self) -> None:
        self.assertTrue(px68k.px68k_spec_is_well_formed(self.spec))
        mutated = {**self.spec, "targets": ["arm64"]}
        self.assertFalse(px68k.px68k_spec_is_well_formed(mutated))
    ...
    def test_workflow_is_a_migrated_read_only_dispatcher(self) -> None:
        ...
        self.assertIn("--core px68k", workflow)
...
class Px68kCompatibilityTests(unittest.TestCase):
    def test_promoted_compatibility_is_valid_and_canonical(self) -> None:
        compatibility_path = ROOT / "manifests/compatibility/px68k.json"
        ...
        self.assertFalse(
            (ROOT / "manifests/compatibility/pending/px68k.json").exists()
        )


class Px68kContractTests(unittest.TestCase):
    ...
    def test_contract_rejects_a_wrong_language_count(self) -> None:
        ...
        if log is None:
            self.skipTest("no workspace-local px68k build log present")
        tampered = dataclasses.replace(
            px68k.PX68K_LOG_CONTRACT,
            expected_language_counts={"cxx": 7, "c": 54},
        )
        self.assertFalse(
            mixed_language_log_proves_contract(
