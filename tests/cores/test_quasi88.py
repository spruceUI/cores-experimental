"""Focused quasi88 catalog, workflow, contract, and canonical-state tests."""
...
from core_pipeline_lib.contracts import quasi88
...
class Quasi88ManifestTests(unittest.TestCase):
    ...
    def test_catalog_binds_exact_libretro_super_recipe(self) -> None:
        ...
        self.assertEqual("libretro-quasi88", self.spec["build"]["source_dir"])
        self.assertEqual(["arm64", "armhf"], self.spec["targets"])

    def test_spec_is_well_formed_pins_the_catalog_identity(self) -> None:
        self.assertTrue(quasi88.quasi88_spec_is_well_formed(self.spec))
        mutated = {**self.spec, "targets": ["arm64"]}
        self.assertFalse(quasi88.quasi88_spec_is_well_formed(mutated))
    ...
    def test_workflow_is_a_migrated_read_only_dispatcher(self) -> None:
        ...
        self.assertIn("--core quasi88", workflow)
...
class Quasi88CompatibilityTests(unittest.TestCase):
    def test_promoted_compatibility_is_valid_and_canonical(self) -> None:
        compatibility_path = ROOT / "manifests/compatibility/quasi88.json"
        ...
        self.assertFalse(
            (ROOT / "manifests/compatibility/pending/quasi88.json").exists()
        )


class Quasi88ContractTests(unittest.TestCase):
        ...
        for run_id in (SELECTED_RUN, REPRODUCTION_RUN):
            for arch in ("arm64", "armhf"):
                ...
                if log is None:
                    continue
                self.assertTrue(
                    quasi88.quasi88_log_proves_contract(
                        log, CORE_ID, arch, SOURCE_COMMIT, SOURCE_TREE
                    ),
                    f"{run_id}/{arch} did not prove the quasi88 contract",
                )
                proven += 1
        if proven == 0:
            self.skipTest("no workspace-local quasi88 build logs present")
    ...
    def test_contract_rejects_a_wrong_language_count(self) -> None:
        ...
        if log is None:
            self.skipTest("no workspace-local quasi88 build log present")
        tampered = dataclasses.replace(
            quasi88.QUASI88_LOG_CONTRACT,
            expected_language_counts={"cxx": 7, "c": 63},
        )
        self.assertFalse(
            mixed_language_log_proves_contract(
