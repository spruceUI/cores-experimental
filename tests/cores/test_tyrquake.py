"""Focused TyrQuake catalog, workflow, contract, and canonical-state tests."""
...
from core_pipeline_lib.contracts import tyrquake
...
class TyrquakeManifestTests(unittest.TestCase):
        ...
        self.assertEqual(
            {
                ...
                "artifact_name": "tyrquake_libretro.so",
                "source_date_epoch": 1784135314,
            },
            self.spec["build"],
        )
    ...
    def test_spec_is_well_formed_pins_the_catalog_identity(self) -> None:
        self.assertTrue(tyrquake.tyrquake_spec_is_well_formed(self.spec))
        mutated = {**self.spec, "targets": ["arm64"]}
        self.assertFalse(tyrquake.tyrquake_spec_is_well_formed(mutated))
    ...
    def test_workflow_is_a_migrated_read_only_dispatcher(self) -> None:
        ...
        self.assertIn("--core tyrquake", workflow)
...
class TyrquakeCompatibilityTests(unittest.TestCase):
    def test_promoted_compatibility_is_valid_and_canonical(self) -> None:
        compatibility_path = ROOT / "manifests/compatibility/tyrquake.json"
        ...
        self.assertFalse(
            (ROOT / "manifests/compatibility/pending/tyrquake.json").exists()
        )
...
class TyrquakeContractTests(unittest.TestCase):
        ...
        for run_id in (SELECTED_RUN, REPRODUCTION_RUN):
            for arch in ("arm64", "armhf"):
                ...
                if log is None:
                    continue
                self.assertTrue(
                    tyrquake.tyrquake_log_proves_contract(
                        log, CORE_ID, arch, SOURCE_COMMIT, SOURCE_TREE
                    ),
                    f"{run_id}/{arch} did not prove the TyrQuake contract",
                )
                proven += 1
        if proven == 0:
            self.skipTest("no workspace-local TyrQuake build logs present")
    ...
    def test_contract_rejects_a_wrong_compile_count(self) -> None:
        ...
        if log is None:
            self.skipTest("no workspace-local TyrQuake build log present")
        tampered = dataclasses.replace(
            tyrquake.TYRQUAKE_LOG_CONTRACT, expected_compile_count=999
        )
        self.assertFalse(
            c_only_log_proves_contract(
