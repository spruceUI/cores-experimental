...
from core_pipeline_lib.contracts import mixed_language, vice_xvic
...
ORACLE_FIXTURE = pipeline.load_json(
    ROOT / "tests/fixtures/per-core-oracles/vice_xvic.json"
)
...
def oracle_log_path(run_id: str, architecture: str) -> Path:
    return (
        ...
        / "runs"
        / run_id
        / vice_xvic.VICE_XVIC_CORE_ID
        / architecture
        / "build.log"
    )


def build_vice_xvic_log_fixture(
    architecture: str,
) -> tuple[
    ...
    version_token = vice_xvic.VICE_XVIC_NATIVE_GIT_VERSION_LOG_TOKEN
    core_token = vice_xvic.VICE_XVIC_CORE_NAME_LOG_TOKEN
    machine_token = vice_xvic.VICE_XVIC_MACHINE_COMPILE_TOKEN
    compile_lines = (
        (
    ...
    parsed_invocations = [
        mixed_language.mixed_language_compile_invocation(
            ...
            vice_xvic.VICE_XVIC_SEMANTIC_PATH_ALIASES,
        )
        for line in compile_lines
    ]
    if any(invocation is None for invocation in parsed_invocations):
        raise AssertionError("failed to construct VICE xvic compile fixture")
    invocations = [
        invocation
    ...
    link_argv = (
        cxx_compiler,
        "-o",
        vice_xvic.VICE_XVIC_BUILD_ARTIFACT_NAME,
        *raw_objects,
        *vice_xvic.VICE_XVIC_EXPECTED_LINK_OPTIONS,
    )
    ordered_link_sha256 = ordered_command_argv_sha256(link_argv)
    if ordered_link_sha256 is None:
        raise AssertionError("failed to construct VICE xvic link fixture")
    contract = replace(
        vice_xvic.VICE_XVIC_LOG_CONTRACT,
    ...
    log = (
        "\n".join(
            (
                ...
                vice_xvic.VICE_XVIC_GIT_ABBREV_MARKER,
                vice_xvic.VICE_XVIC_NATIVE_VERSION_MARKER,
                vice_xvic.VICE_XVIC_CFLAGS_MARKER,
                *compile_lines,
                " ".join(link_argv),
            )
        )
...
def synthetic_contract(
    ...
    with mock.patch.object(
        vice_xvic,
        "VICE_XVIC_EXPECTED_COMPILE_COUNT",
...
class ViceXvicModuleTests(unittest.TestCase):
    def contract_arguments(
        self,
        build_log_text: str,
        architecture: str = "arm64",
    ) -> tuple[str, str, str, str, str]:
        identity = vice_xvic.VICE_XVIC_NATIVE_GIT_VERSION_SPEC_IDENTITY
        return (
            build_log_text,
            vice_xvic.VICE_XVIC_CORE_ID,
            architecture,
    ...
    def test_workflow_is_exact_publication_disabled_template(self) -> None:
        ...
        workflow = (
            ROOT / ".github" / "workflows" / "build-vice_xvic.yml"
        ).read_text(encoding="utf-8")
        self.assertEqual(template.replace("gearcoleco", "vice_xvic"), workflow)
        self.assertEqual(1, workflow.count("--core vice_xvic"))
    ...
    def test_native_short10_epoch_recipe_is_exact_and_sanitized(self) -> None:
        ...
        spec = catalog["cores"][vice_xvic.VICE_XVIC_CORE_ID]
        source = {
        ...
        expected = {
            ...
            "metadata": {
                ...
                "artifact_name": "vice_xvic_libretro.info",
            },
            "targets": ["arm64", "armhf"],
        }
        self.assertEqual(expected, spec)
        self.assertTrue(vice_xvic.vice_xvic_spec_is_well_formed(spec))
        self.assertTrue(
            pipeline.native_git_version_short10_spec_is_well_formed(
                spec, vice_xvic.VICE_XVIC_CORE_ID
            )
        )
        self.assertEqual(
            version,
        ...
        self.assertIn(vice_xvic.VICE_XVIC_GIT_ABBREV_MARKER, shell)
        self.assertIn(
        ...
        build_script = pipeline.container_build_script(
            vice_xvic.VICE_XVIC_CORE_ID, "arm64", spec, catalog["resolver"]
        )
        ...
        build_position = build_script.index("./libretro-build.sh vice_xvic")
    ...
    def test_catalog_identity_epoch_and_copy_mutations_fail_closed(self) -> None:
        ...
        core_id = vice_xvic.VICE_XVIC_CORE_ID
        mutations = (
            mutation(
                "source-commit",
            ...
            mutation(
                "xvic-uses-x64-resolver-key",
                lambda changed: changed["cores"][core_id]["build"].update(
                    {"source_key": "vice_x64"}
                ),
            ),
            mutation(
                "xvic-uses-x64-artifact",
                lambda changed: changed["cores"][core_id]["build"].update(
                    {
        ...
        for label, changed in mutations:
            with self.subTest(mutation=label):
                self.assertFalse(
                    vice_xvic.vice_xvic_spec_is_well_formed(
    ...
    def test_golden_source_build_and_epoch_fail_closed(self) -> None:
        ...
        self.assertTrue(
            vice_xvic.vice_xvic_golden_source_is_well_formed(
                vice_xvic.VICE_XVIC_CORE_ID, source
            )
        )
        self.assertTrue(
            vice_xvic.vice_xvic_golden_build_contract_is_well_formed(
                build,
                spec["source"]["commit"],
                vice_xvic.VICE_XVIC_CORE_ID,
                source,
        ...
        for label, changed_source in source_mutations.items():
            with self.subTest(source_mutation=label):
                self.assertFalse(
                    vice_xvic.vice_xvic_golden_source_is_well_formed(
                        vice_xvic.VICE_XVIC_CORE_ID, changed_source
                    )
                )
                self.assertFalse(
                    pipeline.native_git_version_golden_source_is_well_formed(
    ...
    def test_short10_schema_contract_is_exact_and_disjoint(self) -> None:
        ...
        golden_build = branches[vice_xvic.VICE_XVIC_CORE_ID]["properties"][
            "build"
        ]
    ...
    def test_production_contract_constants_match_reviewed_oracle(self) -> None:
        ...
        self.assertEqual(
            ...
            vice_xvic.VICE_XVIC_EXPECTED_COMPILE_COUNT,
        )
        self.assertEqual(
            contract["language_counts"],
            vice_xvic.VICE_XVIC_EXPECTED_LANGUAGE_COUNTS,
        )
        self.assertEqual(
            contract["source_suffix_counts"],
            vice_xvic.VICE_XVIC_EXPECTED_SOURCE_SUFFIX_COUNTS,
        )
        self.assertEqual(
            contract["compile_pair_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_COMPILE_PAIR_SHA256,
        )
        self.assertEqual(
            contract["compile_invocation_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_COMPILE_INVOCATION_SHA256,
        )
        self.assertEqual(
            contract["link_object_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_LINK_OBJECT_SHA256,
        )
        self.assertEqual(
            contract["raw_link_object_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_RAW_LINK_OBJECT_SHA256,
        )
        self.assertEqual(
            contract["ordered_link_argv_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_ORDERED_LINK_ARGV_SHA256,
        )
        self.assertEqual(
            contract["diagnostic_lines_sha256"],
            vice_xvic.VICE_XVIC_EXPECTED_DIAGNOSTIC_LINES_SHA256,
        )

    def test_synthetic_logs_prove_both_architecture_contracts(self) -> None:
        for architecture in ("arm64", "armhf"):
            contract, _link_argv, log = build_vice_xvic_log_fixture(
                architecture
            )
    ...
    def test_registry_identity_is_owned_by_vice_xvic(self) -> None:
        contract = core_log_contract_for(vice_xvic.VICE_XVIC_CORE_ID)
        self.assertIsNotNone(contract)
        assert contract is not None
        self.assertEqual("vice-xvic-mixed-language-v1", contract.contract_id)
        self.assertEqual("vice_xvic_log_proves_contract", contract.proof_name)
        self.assertEqual(
            frozenset({vice_xvic.VICE_XVIC_CORE_ID}), contract.core_ids
        )

    def test_synthetic_log_rejects_contract_mutations(self) -> None:
        contract, link_argv, log = build_vice_xvic_log_fixture("arm64")
        arguments = self.contract_arguments(log)[1:]
        head = vice_xvic.VICE_XVIC_SOURCE_HEAD_MARKER
        config = vice_xvic.VICE_XVIC_GIT_ABBREV_MARKER
        marker = vice_xvic.VICE_XVIC_NATIVE_VERSION_MARKER
        version_token = vice_xvic.VICE_XVIC_NATIVE_GIT_VERSION_LOG_TOKEN
        core_token = vice_xvic.VICE_XVIC_CORE_NAME_LOG_TOKEN
        machine_token = vice_xvic.VICE_XVIC_MACHINE_COMPILE_TOKEN
        compile_line = next(
        ...
        mutations = {
            ...
            "wrong-core": log.replace(
                core_token, r'-DCORE_NAME=\"x64\"', 1
            ),
            "wrong-machine": log.replace(machine_token, "-D__X64__", 1),
